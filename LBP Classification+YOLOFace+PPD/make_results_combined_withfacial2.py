...
for f in filename_pbar:
    ...
    for idx, confidence_cnt in enumerate(confidence_cnts):
        area = cv2.contourArea(confidence_cnt)
        if area < np.percentile(confidence_cnts_areas, 50): continue
        (x, y, w, h) = cv2.boundingRect(confidence_cnt)
        min_x, max_x = min(x, min_x), max(x + w, max_x)
        min_y, max_y = min(y, min_y), max(y + h, max_y)
    result7 = color.copy()
    ground_truth_bbox = ground_truth_bbox_dict[f]
    result7 = cv2.rectangle(result7, (min_x, min_y),
                            (max_x, max_y), (0, 0, 255), thickness=3)
    result7_iou = calc_iou([min_x, min_y, max_x, max_y],
                           [ground_truth_bbox[0], ground_truth_bbox[1], ground_truth_bbox[0] + ground_truth_bbox[2],
                            ground_truth_bbox[1] + ground_truth_bbox[3]])
    result7_ious.append(result7_iou)
    cv2.imwrite('test_results/' + f.split('.')[0] + '_result7.png', result7)
