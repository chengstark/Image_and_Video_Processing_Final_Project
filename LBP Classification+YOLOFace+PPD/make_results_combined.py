...
for f in filename_pbar:
    ...
    weighted_confidence_recorder = np.zeros((len(range(0, img.shape[0], step)), len(range(0, img.shape[1], step))))
        ...
        for x in range(0, img.shape[1], step):
            ...
            if window.shape[0] == size and window.shape[1] == size:
                ...
                if pred > thresh:
                    ...
                    confidence_recorder[y_idx][x_idx] += pred
                    weighted_confidence_recorder[y_idx][x_idx] += density_weighted_confidence
    ...
    for y in range(weighted_confidence_recorder.shape[0]):
        for x in range(weighted_confidence_recorder.shape[1]):
            ...
            confidence_overlay = cv2.rectangle(confidence_overlay, (coord_x, coord_y),
                                                        (coord_x + size, coord_y + size),
                                                        c, thickness=-1)

    weighted_confidence_recorder = weighted_confidence_recorder / np.max(weighted_confidence_recorder)
    unique_weighted_confidence = np.unique(weighted_confidence_recorder, return_counts=True)[0]
    weighted_cmap_norm = matplotlib.colors.Normalize(vmin=np.min(unique_weighted_confidence), vmax=np.max(unique_weighted_confidence))
    confidence_base = color.copy().astype(np.float32)
    weighted_confidence_overlay = np.zeros_like(color)
    confidence_overlay_adder = np.zeros_like(img).astype(np.float32)
    confidence_overlay_bw = np.zeros_like(img)
    for y in range(weighted_confidence_recorder.shape[0]):
        for x in range(weighted_confidence_recorder.shape[1]):
            neighbour_x = x - n_neighbours
            confidence = weighted_confidence_recorder[y][x]
            coord_y = y * step
            coord_x = x * step
            ...
            weighted_confidence_overlay = cv2.rectangle(weighted_confidence_overlay, (coord_x, coord_y), (coord_x + size, coord_y + size),
                                                        c, thickness=-1)
    ...
    confidence_overlay_wcnt = weighted_confidence_overlay.copy()
    ...
    cv2.imwrite('test_results/' + f.split('.')[0] + '_weighted_confidence_heat.png', weighted_confidence_overlay)
    cv2.imwrite('test_results/' + f.split('.')[0] + '_weighted_confidence_heat_wcnt.png', confidence_overlay_wcnt)
    ...
    ppd_iou = calc_iou([max_confidence_bbox[0], max_confidence_bbox[1], max_confidence_bbox[0] + max_confidence_bbox[2],
    ...
    vote_iou = calc_iou([x, y, x+w, y+h],
                   [ground_truth_bbox[0], ground_truth_bbox[1], ground_truth_bbox[0] + ground_truth_bbox[2],
                    ground_truth_bbox[1] + ground_truth_bbox[3]])
    vote_ious.append(vote_iou)
    #     max_iou_bbox = None
    #     max_iou_cnt = None
    #     max_iou = -999
